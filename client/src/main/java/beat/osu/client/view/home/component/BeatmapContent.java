...
public class BeatmapContent extends ScrollPane {
    ...
    private static final double BASE_CARD_WIDTH = ScreenManager.SCREEN_WIDTH * 0.60;
    private static final double MAX_TRANSLATION_X = -90.0;
    private static final double TRANSLATION_CURVE_STRENGTH = 1.0;
    ...
    private void createAndPositionCard(int index) {
        ...
        Platform.runLater(this::applyCentermostScrollEffect);
    }
    ...
    public void triggerInitialSelection() {
        if (!filteredBeatmaps.isEmpty() && selectedBeatmap != null && !hasTriggeredInitialSelection) {
            ...
            Platform.runLater(() -> {
                applyCentermostScrollEffect();
                Timeline delayedEffect = new Timeline(new KeyFrame(
                    Duration.millis(50),
                    e -> applyCentermostScrollEffect()
                ));
                delayedEffect.play();
            });
        }
    ...
    private void applyCentermostScrollEffect() {
        ...
        double baseTranslation = ScreenManager.SCREEN_WIDTH * 0.16;
        ...
        for (Map.Entry<Integer, BeatmapCard> entry : renderedCards.entrySet()) {
            ...
            double easingFactor = Math.pow(1.0 - normalizedDistance, TRANSLATION_CURVE_STRENGTH);
            double curveTranslationX = MAX_TRANSLATION_X * easingFactor;

            double totalTranslation = baseTranslation + curveTranslationX;

            card.setCurveTranslation(totalTranslation);
        }
    }
}
