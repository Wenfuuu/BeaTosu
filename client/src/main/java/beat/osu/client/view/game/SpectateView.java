...
public class SpectateView extends Page implements GameEventListener, CoordinateConverter {
    ...
    private void createSpectatePane() {
        spectatePane = new Pane();
        spectatePane.setStyle("-fx-background-color: rgba(0, 0, 0, 0.1);");
        for (HitObject hitObject : sm.getHitObjects()) {
    ...
    private void showHitImage(HitObject hitObject, HitResult hitResult, boolean perfectCombo,
        ...
        spectatePane.getChildren().add(hitImageView);
    ...
    private void showAdditionalSpinImage(HitObject hitObject, int additionalSpins) {
        ...
        spectatePane.getChildren().add(spinScoreContainer);
    ...
    private void showArrowWarning() {
        ...
        for (int i = 0; i < 8; i++) {
            double startTime = i * 125;
            double fadeInTime = startTime + 40;
            double fadeOutTime = startTime + 85;
